...
void sink_list(pa_context *context, const pa_sink_info *info, int eol,
               void *userdata) {
  assert(context);
  if (eol < 0) {
  ...
  for (int chan = 0; chan < info->volume.channels; ++chan) {
    ...
    const char *pos_name =
        pa_channel_position_to_pretty_string(info->channel_map.map[chan]);
    for (const CompiledChannelMapping &mapping : mappings) {
      if (const int regex_failed =
              regexec(&mapping.name, pos_name, 0, NULL, 0)) {
  ...
  for (int chan = 0; chan < info->volume.channels; ++chan) {
    ...
    const double normal_volume =
        info->volume.values[chan] / (double)PA_VOLUME_NORM;
    printf("%20s - %s (%0.5f -> %0.5f). targ: %0.5f\n", pos_name, vol_val,
           pa_sw_volume_to_linear(info->volume.values[chan]), normal_volume,
           channel_target[chan]);
  ...
  if (target_mean < 0) {
    fprintf(stderr,
            "warning: we decided to go negative (%0.2f); muting instead\n",
            target_mean);
    target_mean = 0;
  }
  ...
  for (int chan = 0; chan < info->volume.channels; ++chan) {
    ...
    if (factor < target_mean / 4) {
      ...
      fprintf(stderr,
              "warning: clamping channel %d up from %0.5f to %0.5f to prevent "
              "it sounding like ass\n",
  ...
  pa_operation_unref(pa_context_set_sink_volume_by_index(context, info->index,
                                                         &v, volume_cb, NULL));
  ++pending_operations;
}
...
int main(int argc, char *argv[]) {
  ...
  while (true) {
    static struct option long_options[] = {{"sink", required_argument, 0, 0},
    ...
    case '?':
      fprintf(stderr,
              "Usage: %s [--sink regex-for-sink] [--balance "
              "balance-specification] command\n",
              argv[0]);
      goto done;
  ...
  if (optind == argc) {
    ...
    if (len && '-' == val[len - 1]) {
      ...
      fprintf(stderr,
              "argument must have a trailing + or - (for up or down): '%s'\n",
              val);
      goto done;
    }
